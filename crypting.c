...
int calculator_proccess(char *argv[], int proccess_id) 
{
	...
	if (-1 == register_decrypt_msg(num_keys, &decrypt_msg, &MPI_DECRYPT_MSG_T)) {
		fprintf(stderr, "%s\n", "calculator_proccess: register_decrypt_msg");
		return -1;
	}

	if (-1 == register_data_msg(num_keys, &data_msg, &MPI_DATA_MSG_T)) {
		fprintf(stderr, "%s\n", "calculator_proccess: register_data_msg");
		return -1;
	}
...
int assign_key_to_proccess(int proc_id, key_table_t k_table, proc_table_t p_table, int num_keys, int num_procs) 
{
	...
	if (1 == keys_without_procs_flag) {
		...
		if (-1 == register_decrypt_msg(num_keys, &decrypt_msg, &MPI_DECRYPT_MSG_T)) {
			fprintf(stderr, "%s\n", "assign_key_to_proccess: register_decrypt_msg");
			return -1;
		}
...
{	
	...
	return 1; // All the proccesses are occupied
}
 ...
 *			Otherwise, returns 1       *
 ***************************************/
int register_key_type(int num_keys, key_data_t* data, MPI_Datatype* MPI_Type) 
{
	...
	if (0 == num_keys) {
		fprintf(stderr, "%s\n", "register_key_type: num_keys is 0");
		return -1;
	}
 ...
 *  Return: in case of error -1        *
 			Otherwise, returns 1       *
 ***************************************/
int register_decrypt_msg(int num_keys, msg_decrypt_t* data, MPI_Datatype* MPI_Type)  
{
	...
	if (0 == num_keys) {
		fprintf(stderr, "%s\n", "register_decrypt_msg: num_keys is 0");
		return -1;
	}

	/* registerS pMPI_KEY_T */
	if (-1 == register_key_type(num_keys, &(data->key), &MPI_KEY_T)) {
		fprintf(stderr, "%s\n", "register_decrypt_msg: ERROR in register_key_type");
		return -1;
	}
 ...
 *  Return: in case of error -1        *
 			Otherwise, returns 1       *
 ***************************************/
int register_data_msg(int num_keys, msg_data_t* data, MPI_Datatype* MPI_Type) 
{
	...
	if (0 == num_keys) {
		fprintf(stderr, "%s\n", "register_data_msg: num_keys is 0");
		return -1;
	}

	/* registerS pMPI_KEY_T */
	if (-1 == register_key_type(num_keys, &(data->key), &MPI_KEY_T)) {
		fprintf(stderr, "%s\n", "register_data_msg: ERROR in register_key_type");
		return -1;
	}
